...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	style="margin-top: 10px;">
	...
	<c:forEach var="i" items = "${list}" varStatus="status">
		...
		<td align="center">
			<a href="/getPurchase.do?tranNo=${tranNoList[status.index]}">${no}</a>
		...
		<td></td>
		<td align="left">
			<c:if test="${tranCodeList[status.index] eq '2'}">
				<a href="/updateTranCode.do?tranNo=${tranNoList[status.index]}&tranCode=3&currentPage=${resultPage.currentPage}">물건도착</a>    
			</c:if>
		</td>  
		</tr>
		...
		<td align="center">
		  ...
		  <input type="hidden" id="currentPage" name="currentPage" value=""/>
		  	...
		  	<c:if test="${resultPage.currentPage > resultPage.pageUnit}">
		  		<a href="/listPurchase.do?currentPage=${resultPage.currentPage-1}">◀ 이전</a>
		  	</c:if>
		  	
		  	<c:forEach var="i" begin="${resultPage.beginUnitPage}" end="${resultPage.endUnitPage}">
		  		<c:if test="${resultPage.currentPage eq i}">
		  			<U><B><a href="/listPurchase.do?currentPage=${i}">${i}</a></B></U>	
		  		</c:if>
		  	...
		  	<c:if test="${resultPage.endUnitPage < resultPage.maxPage}">
		  		<a href="/listPurchase.do?currentPage=${resultPage.endUnitPage+1}">다음 ▶</a>
		  	</c:if>
